...
-- 성별(남자/여자) 별 사원수 조회
SELECT emp_no,
if(substr(emp_no,8, 1) =1 ,'남','여'),count(*)
FROM employee
GROUP BY emp_no;
...
FROM  employee
GROUP BY dept_code, count(bonus);
...
/*
    ...
    MINUS (차집합) : 선행 쿼리문의 결과값에서 후행 쿼리문의 결과값을 뺀 나머지 결과값만 추출(MySQL)
		--> INTERSECT, MINUS : WHERE절에서 AND연산자를 사용해서 처리 가능!
*/
