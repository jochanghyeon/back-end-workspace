/*
    ...
    1) INSERT INTO 테이블명 VALUES (값, 값, ...);
     ...
     - 컬럼 순번을 지켜서 VALUES 값을 나열해야 한다.alter
...
-- emp_dept 테이블에 employee에서 부서코드가 D1인 직원의 사번, 이름, 부서코드, 입사일
-- emp_manager 테이블에 employee에서 부서 코드가 D1인 직원의 사번, 이름, 관리자, 사번추가
...
/*
    ...
    -사실 서브쿼리 사용가능! 잘 쓰이지도 않고 버전마다 상황이 다름
*/
...
from kh.employee
join kh.deaprtment ON (dept_code = dept_id)
join kh.location ON (local_code = location_id)
where local_name like 'AISA%');
