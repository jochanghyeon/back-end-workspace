/*
    ...
    - 조회 결과는 하나의 결과문(RESULT SET)으로 나옴
...
SELECT emp_id, emp_name, dept_code, dept_title as '소속팀'
...
FROM employee, job
WHERE job_code = job_code; -- ambiguous : 애매한, 오모한 / 에러 발생!!
...
-- 직급이 대리인 사원의 사번(emp_id), 
-- 이름(emp_name), 직급명(job_name), 급여(salary) 조회
...
SELECT * FROM employee; -- dept_code,emp_id, emp_name, bonus
...
SELECT dept_title, dept_id, local_code, local_name
FROM department
...
*/
SELECT * FROM employee; -- emp_name salary 랑연관
...
-- 			사수사번(emp_id), 사수명(emp_name), 사수부서코드(dept_code)

SELECT e.emp_id"사원사번", e.emp_name"사원명", e.dept_code"사원부서코드", e.manager_id"사수사번",
 m.emp_id"사수사번", m.emp_name"사원명", m.dept_code"사수부서코드"
FROM employee e
...
-- 사원명, 부서명 조회(employee - emp_name, department -dept_titel)
-- >> where 구문
...
-- emp_no LIKE '7______-2%';

-- 3. 보너스를 받은 직원들의 직원명, 보너스, 연봉, 부서명, 근무지역 조회
SELECT emp_name, bonus, (salary + salary * bonus)*12 as 연봉, dept_title, local_name
FROM employee
JOIN job USING(job_code)
JOIN department ON(dept_code = dept_id)
JOIN location ON (location_id = local_code)
...
-- 	직원명, 직급명, 급여 조회
SELECT job_code, emp_id, emp_name, dept_title ,salary,bonus
FROM employee
JOIN job USING(job_code)
JOIN department ON(dept_code = dept_id)
...
SELECT emp_id, emp_name, dept_title, job_name, local_code, national_name, sal_level
FROM employee
JOIN job USING(job_code)
JOIN department ON(dept_code = dept_id)
JOIN location ON (location_id = local_code)
