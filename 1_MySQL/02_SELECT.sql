...
/*
	컬럼 산술 연산 
    - SELECT 절에 컬럼명 입력 부분에 산술연산자를 사용하여 결과를 조회할 수있다.
*/
...
/*
    ...
    - 산술연산을 하면 컬럼명이 지저분해진다.
	  이때 컬럼명에 별칭을 부여해서 칼끔하게 보여줄 수 있다.
...
/*
    ...
    - 조회하고자 하는 테이블로 부터 특정 조건에 만족하는 데이터만 조회하고자 할 때 사용
    - 이때 WHERE 절에 조건식을 제시
...
SELECT salary, emp_name, dept_code
FROM employee
WHERE salary >= '4000000';
-- 4. 재직중(ent-yn 컬럼값이 'N')인 사원들의 사번, 사원명, 입사일(hire_date) 조회
SELECT emp_id, hire_date, emp_name, ent_yn
FROM employe
WHERE ent_yn ='N';
...
/*
    ...
    비교대상컬림 LIKE '%문자' => 비교대상컬럼값이 문자로 "끝"나는걸 조회
...
-- 급여(salary), 입사일 조회(hire_date)조회
SELECT emp_name, salary, hire_date
FROM employee
WHERE emp_name LIKE '전%';
-- 이름 중에 '하'가 포함된 사원들의 사원명, 주민번호(emp-no), 전화번호(phone) 조회
...
SELECT phone, emp_id, emp_name, email
FROM employee
WHERE phone LIKE '__1%';

-- 이메일 중 _ 앞글자가 3글자인 사원들의 사번, 이름, 이메일 조회
-- 예) sun_di@kh
-- ESCAPE OPTION : 나만의 와일드 카드를 만들어서 사용! 아무거나 사용가능
SELECT email, emp_id, emp_name
FROM employee
WHERE email LIKE '___ㅁ_%' ESCAPE 'ㅁ';

-- 위의 사원들 이외의 사원들 조회
-- 논리부정연산자 : NOT
SELECT email, emp_id, emp_name
FROM employee
WHERE NOT email LIKE '___ㅂ_%' ESCAPE 'ㅂ';

/*
	IS NULL / IS NOT NULL
    - 컬럼값이 NULL이 있을 경우 NULL 값 비교에 사용되는 연산자
*/
-- 보너스를 받지 않는 사원(보너스 bonus 값이 null)들의
-- 사번(emp_id), 이름,급여,보너스 조회
...
/*
	LIMIT 
    - ORDER BY 절 보다 뒤에 조건을 걷고 싶을 때 사용 
...
SELECT salary, hire_date, bonus, emp_id, emp_name
FROM employee
...
SELECT salary, emp_name, emp_id, bonus, ( SALARY + SALARY * BONUS) * 12 AS 연봉
FROM employee
